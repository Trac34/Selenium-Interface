...
import threading # Another External dependency. Just using a thread to for a polling function
...
So the class does not prevent the use of webdriver functions directly, but rather is a simple mechanism to be used as a building block.  
	...
	"""
	def __init__(self, driver_path, headless=True, proxy=""):
		try:
	...
	def usage(self): ## TODO: Need to update once class is finalized
		...
		print(f"\n{bcolors.BOLD}{bcolors.OKCYAN}Browser.b64sreenshot( path=getcwd(), name='screenshot.png', element=None ){bcolors.ENDC} creates a base64 text file under [path] with [name] or take picture of given [element].")
	...
	def b64screenshot(self, path=getcwd(), name="screenshot.png", element=None):
		p = path +"/"+name
		if element is not None:
			try:
				assert(type(element)) == webdriver.firefox.webelement.FirefoxWebElement
	...
	def startWindowCheck(self):
		...
		try:
			...
			print(f"{bcolors.OKCYAN}[+]{bcolors.ENDC}Thread \'%s\' created with PID: %d" % ( threading.current_thread().name, getpid() ))
		except Exception as e:
